...
while int(pagina_ini) <= int(paginas_total):
    ...
    #Find Cep final (correção de bug)
    cep_ultimo_add = re.split(r'width="80"', html_table)
    cep_ultimo_add = cep_ultimo_add[-1]
    cep_ultimo_add = re.findall(r'(.*?)/tr/table', cep_ultimo_add)
    cep_ultimo_add = [x.strip() for x in cep_ultimo_add]
    cep = cep + cep_ultimo_add

    cep = [x.strip() for x in cep]  #remover espaços no começo e fim ok
    ...
    local_2 = local
    print('LOCAL LISTA', local, len(local))
